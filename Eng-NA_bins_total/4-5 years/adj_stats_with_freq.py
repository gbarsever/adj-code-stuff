...
new_container_counter = Counter(new_container)
...
sem_label_tokens = {}
for token in new_container_counter.keys():
    sem_label = ''
    sub_label = ''
    for y in new_all[1:]:
        if token in y:
    ...
    sem_label_tokens[token] = [new_container_counter[token], sem_label, sub_label]

with open('3_combo_freq.csv', 'wb') as csvfile:
