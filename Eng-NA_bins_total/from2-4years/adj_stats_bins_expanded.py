...
outfile_adjs = open('2-4adjs_exp.txt', 'w')
...
with open('2-4_adjsstats_exp.csv', 'wb') as csv_file:
...
#sorted_x = sorted(subjectivity_dict, key=subjectivity_dict.get, reverse=True)
...
total_tokes = sum(B_L)
for adj in new_dict_all_adjs_d: #goes 1away, 2away
    p2s = []
    count = 0 #f
    total_count = 0 #N
    #print adj, "ADJ"
...
for adj in new_dict_all_adjs_p:
    #if new_dict_all_adjs_p[adj][1] != "other": not fair if using adjs that the other hypotheses arent using
    p2s = []
    count = 0 #f2(adjx)
...
#plt.show()
print range(len(semanticClassDict_p)), semanticClassDict_p.values(), semanticClassDict_p.keys()
